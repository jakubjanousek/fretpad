      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 safe-area-inset-bottom">
        <div className="mx-auto max-w-[1600px] px-4 sm:px-6 lg:px-8 pb-3 sm:pb-4">
          ...
          >
            ...
            {!isMinimal && (
              <div className="hidden xs:flex items-center gap-1.5 justify-self-start">
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => handleTempoChange([Math.max(40, tempo - 5)])}
                  aria-label="Decrease tempo"
                  className="h-8 w-8 rounded-full text-muted-foreground hover:text-stone-200"
                >
