...
> = {
  ...
  "outline-chord-changes": {
    ...
    surface: "bg-stone-950/[0.94] text-stone-50",
  },
  "comp-with-voicings": {
      ...
      >
        ...
        <CardContent className="relative flex h-full flex-col gap-6 p-7 sm:p-8">
          <div className="flex items-start justify-between gap-4">
            <div className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-current/10 bg-white/70 text-current shadow-sm backdrop-blur">
              {visual.icon}
            </div>
            <div className="flex items-center gap-2 rounded-full border border-current/10 bg-white/45 px-3 py-1 text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-current/70 backdrop-blur">
          ...
          <div className="space-y-3">
            <p className="text-[0.7rem] font-semibold uppercase tracking-[0.24em] text-current/60">
              {visual.kicker}
            </p>
            <h2
            ...
            <p className="max-w-md text-sm leading-7 text-current/72 sm:text-[0.98rem]">
              {config.description}
            </p>
            <p className="max-w-lg text-sm leading-7 text-current/62">
              {visual.detail}
            </p>
          </div>

          <div className="mt-auto flex flex-wrap gap-2">
            <span className="rounded-full border border-current/10 bg-white/45 px-3 py-1.5 text-xs font-medium text-current/72">
              {stepCount} steps
            </span>
            <span className="rounded-full border border-current/10 bg-white/45 px-3 py-1.5 text-xs font-medium text-current/72">
          ...
          <div className="flex items-center justify-between border-t border-current/10 pt-5">
            <span className="text-sm font-medium text-current/72">
              Open practice mode
            </span>
            <span className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-current/15 bg-white/60 transition-transform duration-300 group-hover:translate-x-1 group-hover:-translate-y-1">
...
export function Launcher() {
  return (
    <main
      className={`${bodyFont.className} relative min-h-screen overflow-hidden bg-[#f4ecdf] text-[#171411]`}
    >
      <div
        aria-hidden="true"
      ...
      <div
        aria-hidden="true"
        className="absolute left-[-8rem] top-[-7rem] h-72 w-72 rounded-full bg-[radial-gradient(circle,_rgba(245,158,11,0.28)_0%,_rgba(245,158,11,0)_70%)] blur-2xl"
      />
      <div
        aria-hidden="true"
        className="absolute bottom-[-9rem] right-[-8rem] h-96 w-96 rounded-full bg-[radial-gradient(circle,_rgba(56,189,248,0.22)_0%,_rgba(56,189,248,0)_70%)] blur-3xl"
      />

      <div className="relative mx-auto flex min-h-screen w-full max-w-7xl flex-col px-5 py-6 sm:px-8 sm:py-8 lg:px-10">
        <div className="mb-8 flex items-center justify-between gap-4">
          <div className="rounded-full border border-black/10 bg-white/55 px-4 py-2 text-xs font-semibold uppercase tracking-[0.26em] text-stone-700 backdrop-blur">
            FretPad
          </div>
          <div className="hidden rounded-full border border-black/10 bg-white/45 px-4 py-2 text-xs text-stone-600 backdrop-blur sm:block">
            Guitar practice for harmony, hearing, and voicings
          </div>
        </div>

        <section className="grid flex-1 gap-6 lg:grid-cols-[1.03fr_1fr] lg:gap-8">
          <div className="flex flex-col justify-between rounded-[2rem] border border-black/10 bg-[linear-gradient(160deg,rgba(255,251,245,0.9),rgba(242,232,218,0.78))] p-6 shadow-[0_32px_120px_-48px_rgba(75,52,24,0.45)] backdrop-blur-xl sm:p-8 lg:p-10">
            <div>
              <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-black/10 bg-stone-950 px-4 py-2 text-[0.68rem] font-semibold uppercase tracking-[0.24em] text-stone-100">
                <span className="h-2 w-2 rounded-full bg-amber-400" />
                Practice by ear and by sight
              </div>

              <h1
                className={`${displayFont.className} max-w-3xl text-[3.1rem] leading-[0.92] tracking-[-0.03em] text-stone-950 sm:text-[4.6rem] lg:text-[5.6rem]`}
              >
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-stone-950 px-5 py-3 text-sm font-medium text-white transition hover:bg-stone-800 focus-ring"
                >
                  Start with chord changes
                  <ArrowUpRight className="h-4 w-4" />
                </Link>
                <div className="inline-flex items-center rounded-full border border-black/10 bg-white/55 px-5 py-3 text-sm text-stone-700 backdrop-blur">
