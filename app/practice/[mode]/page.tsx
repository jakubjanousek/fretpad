...
import { isValidModeId, PRACTICE_MODES } from "@/lib/modes";
import {
...
export default function PracticePage() {
  ...
  const modeId = modeSlug as PracticeModeId;
  ...
  // Enter mode on mount (once per mode slug)
  const enterMode = useAppStore((state) => state.enterMode);
  const hasEnteredRef = useRef(false);

  // biome-ignore lint/correctness/useExhaustiveDependencies: intentionally run only when modeId changes
  useEffect(() => {
    hasEnteredRef.current = false;
  }, [modeId]);

  // biome-ignore lint/correctness/useExhaustiveDependencies: only enter once per mount/mode change
  useEffect(() => {
